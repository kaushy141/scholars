...
?>
<div class="card mb-4">
  <h5 class="card-header"><?= $data['image'] ? img(array("src"=>base_url().'/'.$data['image'], "class"=>"w-px-30 h-auto rounded-circle")) : ""?> Identity Master Form</h5>
  <?php echo form_open_multipart('admin/secure/master/saveidentity/' . intval($data['id']), ['class' => 'card-body']); ?>
  <div class="row g-3">    
	<div class="col-md-12">
      <label class="form-label" for="mname">Name</label>
      <input type="text" name="name" id="name" maxlength="250" class="form-control" placeholder="Full Identity Name Ex. Adhar Card" value="<?= $data['name'] ?>">
    </div>   
